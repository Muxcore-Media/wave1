...
**Wave 33 (2026-08-09):** Admin login was redirecting browsers to `http://127.0.0.1:9401` on gringotts. Fixed with public/internal auth split — [admin-ui#15](https://github.com/Muxcore-Media/admin-ui/pull/15), [mvp#8](https://github.com/Muxcore-Media/mvp/pull/8) / [mvp#9](https://github.com/Muxcore-Media/mvp/pull/9); redeploy required killing the stale `:8082` process. Verified: `https://admin.gringotts/login` → `https://auth.gringotts/...`. Media SPA already used public auth. Live modules have `MUXCORE_MESH_DIAL_LOCAL` dial helpers; archived dumps still raw `HttpAddr` (ignore). CI: [core#40](https://github.com/Muxcore-Media/core/pull/40) sdk client `x/text@v0.39.0` tidy; [core#41](https://github.com/Muxcore-Media/core/pull/41) `grpc@v1.82.1` (GO-2026-6061) + SLSA predicate for cosign — Vulnerability Scan + SLSA Provenance **green** on PR.

**Wave 32 CI (2026-08-09):** Self-hosted gringotts runner green path: Test/Docker/Coverage; lint (`gocritic`) fixed; `x/text@v0.39.0`; Go `1.26.5`; fuzztime `10s` ([core#36](https://github.com/Muxcore-Media/core/pull/36)–[#39](https://github.com/Muxcore-Media/core/pull/39)). Single-runner queue: prefer tip-of-master CI over graph-update/Dependabot noise.
...
**Remaining packaging:** migrate module workflows to self-hosted labels; GHCR via rootless podman on gringotts; TMDB/WG rotation optional (dev).
...
2. **Self-hosted CI** — gringotts runner serves `Muxcore-Media/core`; point module workflows at `self-hosted` (see `GOLDEN_CI.yml.template`) instead of restoring hosted-minutes billing.
